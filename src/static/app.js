let activitiesList, activitySelect, messageDiv;
    ...
    Object.entries(activities).forEach(([name, details]) => {
      ...
      activityCard.innerHTML = `
        ...
        <div class="participants">
          <h5>Participants</h5>
          <ul>
            ${details.participants.map(participant => `<li class="participant-item">${participant} <span class='delete-icon' onclick='deleteParticipant("${participant}")'>🗑️</span></li>`).join('')}
          </ul>
        </div>
      `;
...
async function deleteParticipant(email) {
  // Find the activity name from the clicked element
